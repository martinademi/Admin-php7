    ...
    function datatable_DriverAcceptanceRate() {
        ...
        foreach ($aaData as $res) {
            if ($res['accountType'] == 1) {
                $res['accountType'] = "Freelancer";
            } else {
                $res['accountType'] = "Operator";
            }
...
//        print_r($cityId);die;
        if ($cityId) {
            $cityData1 = $this->mongo_db->where(array('city_ID' => $cityId))->get('zones');
    ...
    function acceptdrivers($id, $company_id, $planID, $planName, $operatorName, $operatorId, $storeName, $storeId, $planActiveDate, $zoneIds, $ban, $cityId, $cityName) {
        ...
        if ($operatorId) {
            ...
            if (isset($masterData['planActivedOn']) && $masterData['planActivedOn'] != '')
                $result = $this->mongo_db->where(array('_id' => new MongoDB\BSON\ObjectID($id)))->set(array('status' => 8, 'statusMsg' => "logged out", 'planName' => $planName, 'planID' => new MongoDB\BSON\ObjectID($planID), 'operatorId' => $operatorId, 'operatorName' => $operatorName, 'zoneIds' => $zoneIds, 'accountType' => 2))->update('driver');
            else
                $result = $this->mongo_db->where(array('_id' => new MongoDB\BSON\ObjectID($id)))->set(array('status' => 8, 'statusMsg' => "logged out", 'accountType' => 2, 'planActivedOn' => strtotime($planActiveDate), 'planName' => $planName, 'planID' => new MongoDB\BSON\ObjectID($planID), 'operatorId' => $operatorId, 'operatorName' => $operatorName, 'zoneIds' => $zoneIds))->update('driver');
        }
        ...
        if ($storeId == '' && $operatorId == '' || $storeId == null && $operatorId == null) {
            $result = $this->mongo_db->where(array('_id' => new MongoDB\BSON\ObjectID($id)))->set(array( 'planName' => $planName, 'planID' => new MongoDB\BSON\ObjectID($planID),'status' => 8, 'statusMsg' => "logged out", 'accountType' => 1, "approvedTimeStamp" => time(), "approvedISODate" => $this->mongo_db->date($timestamp)))->update('driver');
        }
        ...
        switch ($driverType) {
            ...
            case '':
                if ($plan == '') {
                    ...
                    $data['New'] = $this->mongo_db->where(array('status' => 1, 'accountType' => 3))->count('driver');
                    $data['Approved'] = $this->mongo_db->where(array('accountType' => 3, 'status' => array('$in' => [2, 3, 4, 8, 9])))->count('driver');
                    $data['Accepted'] = $this->mongo_db->where(array('accountType' => 3, 'status' => array('$in' => [3, 4])))->count('driver');
                    $data['Inactive'] = $this->mongo_db->where(array('accountType' => 3, 'status' => array('$in' => [2, 7])))->count('driver');
                    $data['Rejepted'] = $this->mongo_db->where(array('accountType' => 3, 'status' => 6))->count('driver');
                    $data['Banned'] = $this->mongo_db->where(array('accountType' => 3, 'status' => 7))->count('driver');
                    $data['Online'] = $this->mongo_db->where(array('accountType' => 3, 'status' => 3))->count('driver');
                    $data['offline'] = $this->mongo_db->where(array('accountType' => 3, 'status' => 4))->count('driver');
                    $data['loggedOut'] = $this->mongo_db->where(array('accountType' => 3, 'status' => 8))->count('driver');
                    $data['timedOut'] = $this->mongo_db->where(array('accountType' => 3, 'status' => 9))->count('driver');
                } else {
...
//
//                $data['New'] = $this->mongo_db->where(array('status' => 1, 'accountType' => 1))->count('driver');
//                $data['Approved'] = $this->mongo_db->where(array('status' => array('$in' => [2, 3, 4, 8, 9]), 'accountType' => 1))->count('driver');
//                $data['Accepted'] = $this->mongo_db->where(array('status' => array('$in' => [3, 4]), 'accountType' => 1))->count('driver');
//                $data['Inactive'] = $this->mongo_db->where(array('status' => array('$in' => [2, 7]), 'accountType' => 1))->count('driver');
//                $data['Rejepted'] = $this->mongo_db->where(array('status' => 6, 'accountType' => 1))->count('driver');
//                $data['Banned'] = $this->mongo_db->where(array('status' => 7, 'accountType' => 1))->count('driver');
//                $data['Online'] = $this->mongo_db->where(array('status' => 3, 'accountType' => 1))->count('driver');
//                $data['offline'] = $this->mongo_db->where(array('status' => 4, 'accountType' => 1))->count('driver');
//                $data['loggedOut'] = $this->mongo_db->where(array('status' => 8, 'accountType' => 1))->count('driver');
//                $data['timedOut'] = $this->mongo_db->where(array('status' => 9, 'accountType' => 1))->count('driver');
//
//                break;
//            case '2':
//                if ($operator == '') {
...
//
//                    $data['New'] = $this->mongo_db->where(array('status' => 1, 'accountType' => 2))->count('driver');
//                    $data['Approved'] = $this->mongo_db->where(array('status' => array('$in' => [2, 3, 4, 8, 9]), 'accountType' => 2))->count('driver');
//                    $data['Accepted'] = $this->mongo_db->where(array('status' => array('$in' => [3, 4]), 'accountType' => 2))->count('driver');
//                    $data['Inactive'] = $this->mongo_db->where(array('status' => array('$in' => [2, 7]), 'accountType' => 2))->count('driver');
//                    $data['Rejepted'] = $this->mongo_db->where(array('status' => 6, 'accountType' => 2))->count('driver');
//                    $data['Banned'] = $this->mongo_db->where(array('status' => 7, 'accountType' => 2))->count('driver');
//                    $data['Online'] = $this->mongo_db->where(array('status' => 3, 'accountType' => 2))->count('driver');
//                    $data['offline'] = $this->mongo_db->where(array('status' => 4, 'accountType' => 2))->count('driver');
//                    $data['loggedOut'] = $this->mongo_db->where(array('status' => 8, 'accountType' => 2))->count('driver');
//                    $data['timedOut'] = $this->mongo_db->where(array('status' => 9, 'accountType' => 2))->count('driver');
//                } else {
////                    $respo = $this->datatables->datatable_mongodb('driver', array('status' => 1, 'accountType' => 2, 'companyId' => new MongoDB\BSON\ObjectID($operator)));
//
//                    $data['New'] = $this->mongo_db->where(array('status' => 1, 'accountType' => 2, 'companyId' => new MongoDB\BSON\ObjectID($operator)))->count('driver');
//                    $data['Approved'] = $this->mongo_db->where(array('status' => array('$in' => [2, 3, 4, 8, 9]), 'accountType' => 2, 'companyId' => new MongoDB\BSON\ObjectID($operator)))->count('driver');
//                    $data['Accepted'] = $this->mongo_db->where(array('status' => array('$in' => [3, 4]), 'accountType' => 2, 'companyId' => new MongoDB\BSON\ObjectID($operator)))->count('driver');
//                    $data['Inactive'] = $this->mongo_db->where(array('status' => array('$in' => [2, 7]), 'accountType' => 2, 'companyId' => new MongoDB\BSON\ObjectID($operator)))->count('driver');
//                    $data['Rejepted'] = $this->mongo_db->where(array('status' => 6, 'accountType' => 2, 'companyId' => new MongoDB\BSON\ObjectID($operator)))->count('driver');
//                    $data['Banned'] = $this->mongo_db->where(array('status' => 7, 'accountType' => 2, 'companyId' => new MongoDB\BSON\ObjectID($operator)))->count('driver');
//                    $data['Online'] = $this->mongo_db->where(array('status' => 3, 'accountType' => 2, 'companyId' => new MongoDB\BSON\ObjectID($operator)))->count('driver');
//                    $data['offline'] = $this->mongo_db->where(array('status' => 4, 'accountType' => 2, 'companyId' => new MongoDB\BSON\ObjectID($operator)))->count('driver');
//                    $data['loggedOut'] = $this->mongo_db->where(array('status' => 8, 'accountType' => 2, 'companyId' => new MongoDB\BSON\ObjectID($operator)))->count('driver');
//                    $data['timedOut'] = $this->mongo_db->where(array('status' => 9, 'accountType' => 2, 'companyId' => new MongoDB\BSON\ObjectID($operator)))->count('driver');
//                }
//
//                break;
            case '3':
...
//                    $respo = $this->datatables->datatable_mongodb('driver', array('status' => 1, 'accountType' => 2));
                    ...
                    $data['New'] = $this->mongo_db->where(array('status' => 1, 'accountType' => 3))->count('driver');
                    $data['Approved'] = $this->mongo_db->where(array('status' => array('$in' => [2, 3, 4, 8, 9]), 'accountType' => 3))->count('driver');
                    $data['Accepted'] = $this->mongo_db->where(array('status' => array('$in' => [3, 4]), 'accountType' => 3))->count('driver');
                    $data['Inactive'] = $this->mongo_db->where(array('status' => array('$in' => [2, 7]), 'accountType' => 3))->count('driver');
                    $data['Rejepted'] = $this->mongo_db->where(array('status' => 6, 'accountType' => 3))->count('driver');
                    $data['Banned'] = $this->mongo_db->where(array('status' => 7, 'accountType' => 3))->count('driver');
                    $data['Online'] = $this->mongo_db->where(array('status' => 3, 'accountType' => 3))->count('driver');
                    $data['offline'] = $this->mongo_db->where(array('status' => 4, 'accountType' => 3))->count('driver');
                    $data['loggedOut'] = $this->mongo_db->where(array('status' => 8, 'accountType' => 3))->count('driver');
                    $data['timedOut'] = $this->mongo_db->where(array('status' => 9, 'accountType' => 3))->count('driver');
                } else {
//                    $respo = $this->datatables->datatable_mongodb('driver', array('status' => 1, 'accountType' => 2, 'companyId' => new MongoDB\BSON\ObjectID($operator)));

                    $data['New'] = $this->mongo_db->where(array('status' => 1, 'accountType' => 3, 'storeId' => new MongoDB\BSON\ObjectID($store)))->count('driver');
                    $data['Approved'] = $this->mongo_db->where(array('status' => array('$in' => [2, 3, 4, 8, 9]), 'accountType' => 3, 'storeId' => new MongoDB\BSON\ObjectID($store)))->count('driver');
                    $data['Accepted'] = $this->mongo_db->where(array('status' => array('$in' => [3, 4]), 'accountType' => 3, 'storeId' => new MongoDB\BSON\ObjectID($store)))->count('driver');
                    $data['Inactive'] = $this->mongo_db->where(array('status' => array('$in' => [2, 7]), 'accountType' => 3, 'storeId' => new MongoDB\BSON\ObjectID($store)))->count('driver');
                    $data['Rejepted'] = $this->mongo_db->where(array('status' => 6, 'accountType' => 3, 'storeId' => new MongoDB\BSON\ObjectID($store)))->count('driver');
                    $data['Banned'] = $this->mongo_db->where(array('status' => 7, 'accountType' => 3, 'storeId' => new MongoDB\BSON\ObjectID($store)))->count('driver');
                    $data['Online'] = $this->mongo_db->where(array('status' => 3, 'accountType' => 3, 'storeId' => new MongoDB\BSON\ObjectID($store)))->count('driver');
                    $data['offline'] = $this->mongo_db->where(array('status' => 4, 'accountType' => 3, 'storeId' => new MongoDB\BSON\ObjectID($store)))->count('driver');
                    $data['loggedOut'] = $this->mongo_db->where(array('status' => 8, 'accountType' => 3, 'storeId' => new MongoDB\BSON\ObjectID($store)))->count('driver');
                    $data['timedOut'] = $this->mongo_db->where(array('status' => 9, 'accountType' => 3, 'storeId' => new MongoDB\BSON\ObjectID($store)))->count('driver');
                }
        ...
        switch ($driverType) {
            ...
            case '':
                if ($plan == '') {
                    ...
                    $data['New'] = $this->mongo_db->where(array('status' => 1, 'accountType' => 1))->count('driver');
                    $data['Approved'] = $this->mongo_db->where(array('accountType' => 1, 'status' => array('$in' => [2, 3, 4, 8, 9])))->count('driver');
                    $data['Accepted'] = $this->mongo_db->where(array('accountType' => 1, 'status' => array('$in' => [3, 4])))->count('driver');
                    $data['Inactive'] = $this->mongo_db->where(array('accountType' => 1, 'status' => array('$in' => [2, 7])))->count('driver');
                    $data['Rejepted'] = $this->mongo_db->where(array('accountType' => 1, 'status' => 6))->count('driver');
                    $data['Banned'] = $this->mongo_db->where(array('accountType' => 1, 'status' => 7))->count('driver');
                    $data['Online'] = $this->mongo_db->where(array('accountType' => 1, 'status' => 3))->count('driver');
                    $data['offline'] = $this->mongo_db->where(array('accountType' => 1, 'status' => 4))->count('driver');
                    $data['loggedOut'] = $this->mongo_db->where(array('accountType' => 1, 'status' => 8))->count('driver');
                    $data['timedOut'] = $this->mongo_db->where(array('accountType' => 1, 'status' => 9))->count('driver');
                } else {
//                    $respo = $this->datatables->datatable_mongodb('driver', array('status' => 1, 'planID' => new MongoDB\BSON\ObjectID($plan)));

                    $data['New'] = $this->mongo_db->where(array('status' => 1, 'accountType' => 1, 'planID' => new MongoDB\BSON\ObjectID($plan)))->count('driver');
                    $data['Approved'] = $this->mongo_db->where(array('status' => array('$in' => [2, 3, 4, 8, 9]), 'planID' => new MongoDB\BSON\ObjectID($plan)))->count('driver');
                    $data['Accepted'] = $this->mongo_db->where(array('status' => array('$in' => [3, 4]), 'planID' => new MongoDB\BSON\ObjectID($plan)))->count('driver');
                    $data['Inactive'] = $this->mongo_db->where(array('status' => array('$in' => [2, 7]), 'planID' => new MongoDB\BSON\ObjectID($plan)))->count('driver');
                    $data['Rejepted'] = $this->mongo_db->where(array('status' => 6, 'planID' => new MongoDB\BSON\ObjectID($plan)))->count('driver');
...
//        $data = $this->db->query("select profile_pic,license_pic,license_exp from master where mas_id = '" . $mas_id . "'")->result();
    ...
    function getfreelanceDrivers() {
        $this->load->library('mongo_db');
        $driver = $this->mongo_db->where(array('status' => 2, 'accountType' => 1))->get('driver');
    ...
    function getStoreDrivers() {
        $this->load->library('mongo_db');
        $driver = $this->mongo_db->where(array('status' => 2, 'accountType' => 3, 'storeId' => new MongoDB\BSON\ObjectID($id)))->get('driver');
        $entities = "<option value=''>Select</option>";
        foreach ($driver as $driver) {
            $entities .= "<option value='" . $driver['_id']['$oid'] . "' driverMobile='" . $driver['mobile'] . "' driverName='" . trim($driver['firstName']) . ' ' . trim($driver['lastName']) . "'>" . trim($driver['firstName']) . ' ' . trim($driver['lastName']) . '-' . $driver['email'] . '-' . $driver['mobile'] . "</option>";
        }
...
//                            break;
            ...
            switch ($status) {
                case 3:switch ($driverType) {//online or free
                        case '':if ($plan == '') {
                                $respo = $this->datatables->datatable_mongodb('driver', array('status' => 3, 'accountType' => 3));
                                $respo['status'] = 11;
                                // var_dump($respo);
                                // exit;
                            } else {
                            ...
                            break;
//                        case '1':if ($plan == '') {
//                                $respo = $this->datatables->datatable_mongodb('driver', array('status' => 3, 'accountType' => 1));
//                            } else {
//                                $respo = $this->datatables->datatable_mongodb('driver', array('status' => 3, 'planID' => new MongoDB\BSON\ObjectID($plan)));
//                            }
//                            break;
//                        case '2':if ($plan == '') {
//                                if ($operator == '') {
//                                    $respo = $this->datatables->datatable_mongodb('driver', array('status' => 3, 'accountType' => 2));
//                                } else {
//                                    $respo = $this->datatables->datatable_mongodb('driver', array('status' => 3, 'accountType' => 2, 'companyId' => new MongoDB\BSON\ObjectID($operator)));
//                                }
//                            } else {
//                                $respo = $this->datatables->datatable_mongodb('driver', array('status' => 3, 'planID' => new MongoDB\BSON\ObjectID($plan)));
//                            }
//                            break;
                        ...
                        case '3':
                            if ($store == '') {
                                ...
                                $respo = $this->datatables->datatable_mongodb('driver', array('status' => 3, 'accountType' => 3, 'storeId' => new MongoDB\BSON\ObjectID($store)));
                                $respo['status'] = 11;
                            }
                ...
                case 567:switch ($driverType) {//logged out
                        case '':if ($plan == '') {
                                $respo = $this->datatables->datatable_mongodb('driver', array('status' => 8, 'accountType' => 3));
                                $respo['status'] = 12;
                            } else {
                            ...
                            break;
//                        case '1':if ($plan == '') {
//                                $respo = $this->datatables->datatable_mongodb('driver', array('status' => 8, 'accountType' => 1));
//                            } else {
//                                $respo = $this->datatables->datatable_mongodb('driver', array('status' => 8, 'planID' => new MongoDB\BSON\ObjectID($plan)));
//                            }
//                            break;
//                        case '2':if ($plan == '') {
//                                if ($operator == '') {
//                                    $respo = $this->datatables->datatable_mongodb('driver', array('status' => 8, 'accountType' => 2));
//                                } else {
//                                    $respo = $this->datatables->datatable_mongodb('driver', array('status' => 8, 'accountType' => 2, 'companyId' => new MongoDB\BSON\ObjectID($operator)));
//                                }
//                            } else {
//                                $respo = $this->datatables->datatable_mongodb('driver', array('status' => 8, 'planID' => new MongoDB\BSON\ObjectID($plan)));
//                            }
//                            break;
                        ...
                        case '3':
                            if ($store == '') {
                                ...
                                $respo = $this->datatables->datatable_mongodb('driver', array('status' => 8, 'accountType' => 3, 'storeId' => new MongoDB\BSON\ObjectID($store)));
                                $respo['status'] = 12;
                            }
                ...
                case 30:switch ($driverType) {
                        //offline
                        case '':if ($plan == '') {
                                $respo = $this->datatables->datatable_mongodb('driver', array('status' => 4, 'accountType' => 3));
                                $respo['status'] = 13;
                            } else {
...
//                            break;
//                        case '1':if ($plan == '') {
//                                $respo = $this->datatables->datatable_mongodb('driver', array('status' => 4, 'accountType' => 1));
//                            } else {
//                                $respo = $this->datatables->datatable_mongodb('driver', array('status' => 4, 'planID' => new MongoDB\BSON\ObjectID($plan)));
//                            }
//                            break;
//                        case '2':if ($plan == '') {
//                                if ($operator == '') {
//                                    $respo = $this->datatables->datatable_mongodb('driver', array('status' => 4, 'accountType' => 2));
//                                } else {
//                                    $respo = $this->datatables->datatable_mongodb('driver', array('status' => 4, 'accountType' => 2, 'companyId' => new MongoDB\BSON\ObjectID($operator)));
//                                }
//                            } else {
//                                $respo = $this->datatables->datatable_mongodb('driver', array('status' => 4, 'accountType' => 2, 'planID' => new MongoDB\BSON\ObjectID($plan)));
//                            }
//
//                            break;
                        ...
                        case '3':
                            if ($store == '') {
                                ...
                                $respo = $this->datatables->datatable_mongodb('driver', array('status' => 4, 'accountType' => 3, 'storeId' => new MongoDB\BSON\ObjectID($store)));
                                $respo['status'] = 13;
                            }
                ...
                case 3:switch ($driverType) {
                        case '':if ($plan == '') {
                           ...
                           $respo['status'] = 3;
                                ...
                                $respo = $this->datatables->datatable_mongodb('driver', array('planID' => new MongoDB\BSON\ObjectID($plan), 'status' => array('$in' => [3, 4]), 'accountType' => 1));
                           $respo['status'] = 3;
                                }
                            break;
...
//                'companyName' => $operatorName, 'accountType' => (int) $this->input->post('driverType'), 'zones' => $this->input->post('checkboxs'), "firstName" => $firstname, "lastName" => $last_name,
//                "location" => array(
//                    "longitude" => 0,
//                    "latitude" => 0
...
//        } else
        if ($account_type == 3) {
            $mongoArr = array('registerFrom' => 'web', "memberRenewalDate" => strtotime($memberRenewalDate), 'referralCode' => $getReferralCode,
                'storeName' => $storeName, 'storeId' => new MongoDB\BSON\ObjectID($this->input->post('store_select')), 'cityId' => $cityId, 'cityName' => $cityName, 'accountType' => (int) $this->input->post('driverType'), 'preferredZones' => $this->input->post('checkboxs'), 'serviceZones' => $this->input->post('checkboxs'), "firstName" => $firstname, "lastName" => $last_name,
                "location" => array("longitude" => 0, "latitude" => 0), 'dob' => $date_of_birth, "profilePic" => $this->input->post('driverImage'),
                'status' => 1, 'statusMsg' => "New", 'mobile' => $mobile, 'countryCode' => $countryCode, 'email' => strtolower($email), 'password' => $password2, 'createdDate' => time(), 'createdISODate' => $this->mongo_db->date($timestamp), 'driverLicense' => $license_pic, 'driverLicenseFront' => $license_pic, 'driverLicenseBack' => $license_picBack, 'driverLicenseExpiry' => $expirationrc);
        } else {
            $mongoArr = array('planID' => new MongoDB\BSON\ObjectID($this->input->post('plan')), 'cityId' => $cityId, 'cityName' => $cityName, 'registerFrom' => 'web', 'accountType' => (int) $this->input->post('driverType'),
                ...
                'dob' => $date_of_birth, "profilePic" => $this->input->post('driverImage'), 'status' => 1, 'statusMsg' => "New",
...
//        print_r($mongoArr);die;
    ...
    function editdriverdata() {
	   ...
	   $account_type = 1;
        ...
        if ($profile_pic != ''){
            $mongoArr = array('dob' => $dob, 'mobile' => $mobile, 'countryCode' => $countryCode, 'accountType' => (int) $account_type, "firstName" => $first_name, "lastName" => $last_name, "profilePic" => $profile_pic, 'serviceZones' => $zonesSelected, 'driverLicenseExp' => $expirationrc,'driverLicenseFront'=>$license_pic,'driverLicenseBack'=>$license_picBack);
		} else{
            $mongoArr = array('driverLicenseFront'=>$license_pic,'driverLicenseBack'=>$license_picBack,'dob' => $dob, 'mobile' => $mobile, 'countryCode' => $countryCode, 'accountType' => (int) $account_type, "firstName" => $first_name, "lastName" => $last_name, 'serviceZones' => $zonesSelected, 'driverLicenseExp' => $expirationrc);
		}
        if ($license_pic != ''){
	...
	function editStoreDriverData() {
        ...
        if ($profile_pic != ''){
            $mongoArr = array('dob' => $dob,  'storeId' => new MongoDB\BSON\ObjectID($storeId), 'storeName' => $storeName, "driverLicenseFront"=>$license_pic,"driverLicenseBack"=>$license_picBack,'mobile' => $mobile, 'countryCode' => $countryCode, 'accountType' => (int) $account_type, "firstName" => $first_name, "lastName" => $last_name, "profilePic" => $profile_pic,'preferredZones' => $zonesSelected, 'serviceZones' => $zonesSelected, 'driverLicenseExp' => $expirationrc);
        }else{
            $mongoArr = array('dob' => $dob, 'storeId' => new MongoDB\BSON\ObjectID($storeId),'preferredZones' => $zonesSelected, 'storeName' => $storeName, "driverLicenseFront"=>$license_pic,"driverLicenseBack"=>$license_picBack, 'mobile' => $mobile, 'countryCode' => $countryCode, 'accountType' => (int) $account_type, "firstName" => $first_name, "lastName" => $last_name, 'serviceZones' => $zonesSelected, 'driverLicenseExp' => $expirationrc);
		}
        if ($license_pic != ''){
            $mongoArr['driverLicense'] = $license_pic;
		}
...
//             'companyId' => new MongoDB\BSON\ObjectID($operator)
        ...
        foreach ($aaData as $value) {
            ...
            $arr[] = $value['email'];
            $arr[] = $value['planName'];
            $arr[] = ($value['accountType'] == 2) ? 'N/A' : $value['referralCode'];
