...
//        $count1 = count($cat);
        ...
        switch($status){
            case 0 : 
            $respo = $this->datatables->datatable_mongodb('storeAddOns', array("status"=>0,'storeId' => $this->session->userdata('badmin')['BizId']), 'seqId', 1); //1->ASCE -1->DESC
            break; 

            case 1 : 
            $respo = $this->datatables->datatable_mongodb('storeAddOns', array("status"=>1,'storeId' => $this->session->userdata('badmin')['BizId']), 'seqId', 1); //1->ASCE -1->DESC
            break;  
            
            case 2 : 
            $respo = $this->datatables->datatable_mongodb('storeAddOns', array("status"=>2,'storeId' => $this->session->userdata('badmin')['BizId']), 'seqId', 1); //1->ASCE -1->DESC
            break; 

            case 3 : 
            $respo = $this->datatables->datatable_mongodb('storeAddOns', array("status"=>3,'storeId' => $this->session->userdata('badmin')['BizId']), 'seqId', 1); //1->ASCE -1->DESC
            break;
    ...
    public function getProductsBySerach() {
        ...
        $sRegex = '^'.$sRegex;
        $sRegex = "$sRegex";
        $searchTermsAny[] = array('name.en' => new MongoDB\BSON\Regex($sRegex, "i"),'status'=>1);      
        $searchTerms = array();
...
//        echo '<pre>';       print_r($data);   die;
    ...
    //    language
      $langres=$this->mongo_db->get_where('lang_hlp', array('Active' => 1));
        ...
        //  old code
        //   for($i=0;$i<count($Fdata['addOns']);$i++){        
        //     $Fdata['addOns'][$i]['name'] = array_combine($lanCodeArr, $Fdata['addOns'][$i]['name']);        
        //    } 

        //   foreach($Fdata['addOns'] as $AddOns){   
        //         $addname=(object)$AddOns['name'];          
        //         $id=(string)new MongoDB\BSON\ObjectID();      
        //     $dataAddOn = (object) array("id"=>$id,"name"=>  $addname,"price"=>$AddOns['price']);  
      ...
      if ($AddOnId != '') {
            $res=$this->mongo_db->update('storeAddOns', $Fdata, array("_id" => new MongoId($AddOnId)));
            //print_r($res);die;
            return true;
        } else {
 ...
 /*   function editAddOnData(){
     ...
     foreach ($lang as $lan) {
        ...
        $Fdata['addOnLimit']=(string)$Fdata['addOnLimit'];
