...
//       $this->db->query("update master set countryCode = '".$countryCode."',license_exp = '".date("Y-m-d", strtotime($this->input->post('expirationrc')))."',first_name = '".$first_name."',last_name = '".$last_name."',password = '".md5($password)."',mobile = '".$mobile."',date_of_birth = '".$dob."',account_type = ".$account_type.",company_id = '".$company_id."' where mas_id = ".(int)$driverid."");
        ...
        if ($profile_pic != '')
            $mongoArr = array('driverLicenseBack'=>$driverLicenseBack,'driverLicenseFront'=>$driverLicenseFront,'dob' => $dob, "memberRenewalDate" => strtotime($memberRenewalDate), 'storeId' => new MongoDB\BSON\ObjectID($storeId), 'storeName' => $storeName, 'companyId' => new MongoDB\BSON\ObjectID($company_id), 'companyName' => $operatorName, 'mobile' => $mobile, 'countryCode' => $countryCode, 'driverType' => (int) $account_type, "firstName" => $first_name, "lastName" => $last_name, "profilePic" => $profile_pic, 'zones' => $zonesSelected, 'driverLicenseExp' => $expirationrc,'orderFullfillmentType'=>$orderFulFillmentType,);
        else
            $mongoArr = array('driverLicenseBack'=>$driverLicenseBack,'driverLicenseFront'=>$driverLicenseFront,'dob' => $dob, "memberRenewalDate" => strtotime($memberRenewalDate), 'storeId' => new MongoDB\BSON\ObjectID($storeId), 'storeName' => $storeName, 'companyId' => new MongoDB\BSON\ObjectID($company_id), 'companyName' => $operatorName, 'mobile' => $mobile, 'countryCode' => $countryCode, 'driverType' => (int) $account_type, "firstName" => $first_name, "lastName" => $last_name, 'zones' => $zonesSelected, 'driverLicenseExp' => $expirationrc,'orderFullfillmentType'=>$orderFulFillmentType,);
