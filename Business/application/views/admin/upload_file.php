...
if (!defined('awsAccessKey'))
    define('awsAccessKey', 'AKIAJQOFVZTEZ7BA4B4Q');
if (!defined('awsSecretKey'))
    define('awsSecretKey', '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');

$uploadFile = $tmp1;

//echo $uploadFile . '<br>';
$bucketName = 'postmatesapp';
$folder = "";
//echo json_encode(array('msg' => '2', 'folder' => $folder));
//return;
if ($query && $query['status'] == 'success') {
    try {
        $folder.=$query['country'] . '/' . $query['city'];
    } catch (Exception $e) {
        $folder.=$query['country'] . '/' . $query['city'];
    }
} else {
    try {
        $folder.= $_REQUEST['entityname'];
    } catch (Exception $e) {
        $folder.= $_REQUEST['entityname'];
    }
}
