...
$name = $_FILES['myMainfile']['name']; // filename to get file's extension
$size = $_FILES['myMainfile']['size'];
...
$tmp1 = $_FILES['myMainfile']['tmp_name'];
//$move = move_uploaded_file($tmp1, $file_to_open);

$flag = FALSE;
$imagedetails = getimagesize($_FILES['myMainfile']['tmp_name']);
...
$folder = "images";
    ...
    //echo json_encode(array( 'fileName' => $bucketName . '/' . $folder . '/' . $rename_file, '$name' => $name, '$query' => $query));
...
if ($flag) {
    echo json_encode(array('msg' => '1', 'fileName' => $bucketName . '/' . $folder . '/' . $rename_file, 'Width' => $width, 'Height' => $height));
} else {
