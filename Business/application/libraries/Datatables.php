...
class Datatables {
    ...
    public function getdataFromMongo($aaData) {
        ...
        if ($aaData['count'] == "" || $aaData['count'] == null) {
            $iFilteredTotal = count($aaData); //$this->get_total_results(TRUE);
//            unset($aaData['count']);
        } else {
            $iFilteredTotal = $aaData['count']; //$this->get_total_results(TRUE);
//            unset($aaData['count']);
        }
...
//        $sColumns = array_diff($this->columns, $this->unset_columns);
    ...
    // for aggrigate query
    function datatable_mongodbAggregateold($collection, $queryobj = array(), $countRecordCondition = array()) {
    ...
    //    print_r($searchTerms);die;
       ...
       // end of comment
        ...
        if ($countRecordCondition) {
        $totalrec = $this->ci->mongo_db->aggregate($m_collection, $countRecordCondition);
        foreach ($totalrec as $totalRecords) {
        $totalRecords = json_decode(json_encode($totalRecords), TRUE);
        $totalrec = (int)$totalRecords['count'];

        }
        }

       if (count($searchTerms) > 0) {
        //  $queryobj = array('$and' => array($queryobj, $searchTerms));
        $searchTerms =  array('$match'=>$searchTerms);
       
        $queryobj[1] = $searchTerms;
        } else {
          ...
          // for counting
        //   $rowCount=0;
        //   $cursor = $this->ci->mongo_db->aggregate($m_collection, $queryobj);
        //   foreach($cursor as $row){
        //     $rowCount++;
        //   }

        if (isset($input['iDisplayStart']) && $input['iDisplayLength'] != '-1') {
        array_push($queryobj, array('$skip' => (int) $input['iDisplayStart']),array('$limit' => (int) $input['iDisplayLength']));
       $cursor = $this->ci->mongo_db->aggregate($m_collection, $queryobj);
      // $cursor = $this->ci->mongo_db->aggregate($m_collection, $searchTerms);
        }
       // $totalrec = count($cursor);
        $output = array(
        ...
        function datatable_mongodbAggregate($collection, $queryobj = array(), $countRecordCondition = array()) {
    ...
    function datatable_mongodbold($collection, $queryobj = array(), $filedNameToSort = '', $sortOrder = -1) {
...
//            $cursor->limit($input['iDisplayLength'])->skip($input['iDisplayStart']);
    ...
    function datatable_mongodb($collection, $queryobj = array(), $filedNameToSort = '', $sortOrder = -1, $includeFields = array()) {
...
//                    else
        ...
        if (!empty($searchTermsAny)) {
         ...
         if (count($queryobj) > 0) {
//            $totalrec = $this->ci->mongo_db->where($queryobj)->count($m_collection);
            $totalrec = $this->ci->mongo_db->aggregate($m_collection, array(array('$match' => $queryobj), array(
                    '$group' => array(
...
//            $totalrec = $this->ci->mongo_db->count($m_collection);
             $totalrec = $this->ci->mongo_db->aggregate($m_collection, array(array(
                    '$group' => array(
                        "_id" => 0, 'count' => array('$sum' => 1)
                    )
            )));
        ...
        if (isset($input['iDisplayStart']) && $input['iDisplayLength'] != '-1') {
            if (count($includeFields) > 0){
                $cursor = $this->ci->mongo_db->where($searchTerms)->offset($input['iDisplayStart'])->limit($input['iDisplayLength'])->order_by(array($sortFiled => (int) $sortOrder))->select($includeFields)->get($m_collection);
            }
            else
                $cursor = $this->ci->mongo_db->where($searchTerms)->offset($input['iDisplayStart'])->limit($input['iDisplayLength'])->order_by(array($sortFiled => (int) $sortOrder))->get($m_collection);
//            $cursor->limit($input['iDisplayLength'])->skip($input['iDisplayStart']);
        } else {
//            $cursor = $this->ci->mongo_db->where($searchTerms)->order_by(array($sortFiled => (int) $sortOrder))->get($m_collection);
        }
    ...
    function datatable_mongodbCities($collection, $queryobj = array(), $filedNameToSort = '', $sortOrder = -1) {
...
//                    else
        ...
        $totalrec = $this->ci->mongo_db->where($queryobj)->count($m_collection);
        $totalrecCities = $this->ci->mongo_db->where($queryobj)->get($m_collection);
        $i = 0;
        $j = 0;
        foreach($totalrecCities as $cities){
            $j = $j+1;
            foreach($cities['cities'] as $count){
                $i = $i+1;
            }
        }
