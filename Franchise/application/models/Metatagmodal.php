...
//        print_r($param2);
        ...
        if ($param1 && $param2) {
            $respo = $this->datatables->datatable_mongodb('metaTags', array('franchiseId' => $this->session->userdata('fadmin')['MasterBizId'],'categoryId' => $param1, 'subCategoryId' => $param2), 'seqID', -1); //1->ASCE -1->DESC  
        } else {
            $respo = $this->datatables->datatable_mongodb('metaTags', array('franchiseId' => $this->session->userdata('fadmin')['MasterBizId'],'categoryId' => $param1), 'seqID', -1); //1->ASCE -1->DESC
        }
        $aaData = $respo["aaData"];
        $datatosend = array();
        $sl = 1;
