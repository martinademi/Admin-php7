       ...
       // $_POST['mDataProp_0'] = "storeName";
        ...
        if ($for == 'my') {
            switch ($status) {
                ...
                case 1:
                    $respo = $this->datatables->datatable_mongodb('franchiseManagers', array('franchiseId' =>$id ,'status' => 1), '', -1);
                    break;
                case 2:
                    $respo = $this->datatables->datatable_mongodb('franchiseManagers', array('franchiseId' =>$id ,'status' => 2), '', -1);
                    break;
                case 3:
                    $respo = $this->datatables->datatable_mongodb('franchiseManagers', array('franchiseId' =>$id ,'status' => 3), '', -1);
                    break;
                case 4:
                    $respo = $this->datatables->datatable_mongodb('franchiseManagers', array('franchiseId' =>$id ,'status' => 4), '', -1);
                    break;
            }
        }
        ...
        foreach ($aaData as $value) {
           ...
           // $arr[] = $value['storeName'];
            ...
            $arr[] = $accepts;
            $arr[] = '<button style="width:35px;" class="btn btnedit btn-primary"   value="' . $value['_id']['$oid'] . '"  data-id=' . $value['_id']['$oid'] . ' style="border-radius: 0px;"><i class="fa fa-edit"></i></button>'
                    .'<button style="width:35px;" class="btn btndeviceLogs btn-info "   value="' . $value['_id']['$oid'] . '"  data-id=' . $value['_id']['$oid'] . ' style="border-radius: 0px;"><i class="glyphicon glyphicon-phone"></i></button>'
                    .'<button style="width:35px;" class="btn btnresetpwd "   value="' . $value['_id']['$oid'] . '"  data-id=' . $value['_id']['$oid'] . ' style="border-radius: 0px;"><i class="glyphicon glyphicon-refresh"></i></button>';
...
//        print_r($pass); die;
       ...
       // foreach ($val as $id) {
            $this->mongo_db->where(array('_id' => new MongoDB\BSON\ObjectID($val)))->set(array('password' => $password))->update('franchiseManagers');
            $getData = $this->mongo_db->where(array('_id' => new MongoDB\BSON\ObjectID($val)))->find_one('franchiseManagers');
...
//        print_r($pass);die;
        ...
        $franchiseData = $this->mongo_db->where(array('_id'=> new MongoDB\BSON\ObjectID($franchiseId)))->select(array("franchiseName"=>"franchiseName","cityId"=>"cityId","cityName"=>"cityName"))->get('franchise');
        foreach($franchiseData as $fData){
            $franchiseName = $fData['franchiseName'];
            $cityId = $fData['cityId'];
            $cityName = $fData['cityName'];
...
//        print_r($res);exit();
    ...
    function editManager() {
       ...
       // $storeName = $this->input->post('storeName');
        ...
        $result = array('profilePic'=>$profilePic,'name' => $name, 'email' =>(string)$email,'accepts' => $accepts, 'phone' => $mobile, 'countryCode' => $countryCode, 'franchiseId' => $editfranchiseId);
// print_r($result); die;
    ...
    function getManagersCount() {
        $id = $this->session->userdata('fadmin')['MasterBizId'];
        $this->load->library('mongo_db');
        $data['New'] = $this->mongo_db->where(array('franchiseId' =>$id ,'status' => 1))->count('franchiseManagers');
        $data['loggedin'] = $this->mongo_db->where(array('franchiseId' =>$id ,'status' => 2))->count('franchiseManagers');
        $data['loggedout'] = $this->mongo_db->where(array('franchiseId' =>$id ,'status' => 3))->count('franchiseManagers');
        $data['deleted'] = $this->mongo_db->where(array('franchiseId' =>$id ,'status' => 4))->count('franchiseManagers');
    ...
    function logoutManagers() {
        ...
        echo json_encode(array("msg" => "Selected Manager has been logged-out successfully", "flag" => 0));
        return;
    }
