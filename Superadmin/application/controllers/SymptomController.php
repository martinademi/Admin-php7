...
class symptomController extends CI_Controller {
    ...
    public function __construct() {
        ...
        $this->load->model('Symptommodal');
        
        $language = ($this->session->userdata('lang'))?($this->session->userdata('lang')):'english';
        $this->lang->load('header_lang',$language);
        $this->lang->load('symptom_lang', $language);
       ...
       public function index($status = '') {
        ...
        $data['language'] = $this->Symptommodal->getlanguageText();
        ...
        $data['pagename'] = "Symptoms/symptom";
        $this->load->view("company", $data);
    }
    
     public function data_details($status = '') {
        $this->Symptommodal->data_details($status);
    }
    
     public function addBrand() {
        $this->Symptommodal->addBrand();
    }
     public function getBrand($id = '') {
        $data1 = $this->Symptommodal->getBrand($id);
        echo json_encode(array('data' => $data1));
    }
     public function editBrand() {
        $this->Symptommodal->editBrand();
    }
     public function activateBrand() {
        $this->Symptommodal->activateBrand();
    }
     public function deactivateBrand() {
        $this->Symptommodal->deactivateBrand();
    }

    public function deleteSymtom() {
        $this->Symptommodal->deleteSymtom();
    }
