...
class OrderItemtypecontroller extends CI_Controller {
...
$this->Logoutmodal->logout();
        $this->load->model('OrderItemtypemodal');
        
         $language = ($this->session->userdata('lang'))?($this->session->userdata('lang')):'english';
         $this->lang->load('header_lang',$language);
         $this->lang->load('ordertype_lang', $language);
       ...
       public function index($status = '') {
        ...
        $data['language'] = $this->OrderItemtypemodal->getlanguageText();
        ...
        $data['pagename'] = "OrderItemtype/ordertype";
        $this->load->view("company", $data);
    }
    
     public function data_details($status = '') {
        $this->OrderItemtypemodal->data_details($status);
    }
    
     public function addBrand() {
        $this->OrderItemtypemodal->addBrand();
    }
     public function getBrand($id = '') {
        $data1 = $this->OrderItemtypemodal->getBrand($id);
        echo json_encode(array('data' => $data1));
    }
     public function editBrand() {
        $this->OrderItemtypemodal->editBrand();
    }
     public function activateBrand() {
        $this->OrderItemtypemodal->activateBrand();
    }
     public function deactivateBrand() {
        $this->OrderItemtypemodal->deactivateBrand();
    }

    public function deleteSymtom() {
        $this->OrderItemtypemodal->deleteSymtom();
    }
