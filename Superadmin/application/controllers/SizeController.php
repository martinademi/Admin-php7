...
class SizeController extends CI_Controller {
    ...
    public function __construct() {
        ...
        $this->lang->load('size_lang', $language);
       ...
       public function index($status = '') {
        ...
        $data['language'] = $this->Sizemodal->getlanguageText();
        ...
        $this->table->set_heading($this->lang->line('col_sino'),$this->lang->line('col_Name'),$this->lang->line('col_Cat'),$this->lang->line('col_SubCat'),$this->lang->line('col_SubSubCat'),$this->lang->line('col_Desc'),  $checkbox);
    
        $data['pagename'] = "SizeGroups/sizegroup";
        $this->load->view("company", $data);
    }
    
     public function size_details($status = '') {
        $this->Sizemodal->size_details($status);
    }
    
     public function addSizeGroup() {
        $this->Sizemodal->addSizeGroup();
    }
     public function getSize() {
        $data1 = $this->Sizemodal->getSize();
        echo json_encode(array('data' => $data1));
    }
    public function getSubCategoryData($cid = '',$sid = '') {
     $this->Sizemodal->getSubCategoryData($cid, $sid);
    }
    public function getSubsubCategoryDataList($sid = '',$ssid = '') {
     $this->Sizemodal->getSubsubCategoryDataList($sid, $ssid);
    }
     public function editSize() {
       $this->Sizemodal->editSize();
    }
     public function activateColor() {
        $this->Sizemodal->activateColor();
    }
     public function deactivateColor() {
        $this->Sizemodal->deactivateColor();
    }
