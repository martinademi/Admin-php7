...
class Colorsmodal extends CI_Model {
    ...
    function colors_details($status) {
        ...
        $respo = $this->datatables->datatable_mongodb('colors', array("status" => (int) $status), 'seqId', -1);
        ...
        foreach ($aaData as $value) {
            ...
            $arr[] = $sl++;
            $arr[] = $colorName;
            $arr[] = '<button class="btn btn-primary btnedit cls111" id="btnedit"  value="' . $value['_id']['$oid'] . '"  data-id=' . $value['_id']['$oid'] . ' style="width:35px; border-radius: 25px;"><i class="fa fa-edit" style="font-size:12px;"></i></button>';
