...
class Utilities_m extends CI_Model {
    ...
    public function datatable_canreason($reason, $reasonFor){
        ...
        $this->load->library('table');
        $_POST['iColumns'] = 1;
        $_POST['mDataProp_0'] = "reasons.en";

        $respo = $this->datatables->datatable_mongodb('cancellationReason'.$reason, array('res_for' => $reasonFor), '', 1);
        $aaData = $respo["aaData"];
        $datatosend = array();
        ...
        foreach ($aaData as $value) {
            ...
            $arr[] = $index++;
            $arr[] = '<a class="cancelReson"  style="cursor: pointer" data-id="' . $value['_id']['$oid'] . '">' . $value['reasons']['en'] . '</a>';
    ...
    function deleteCan() {
        ...
        foreach ($ids as $id) {
            $this->mongo_db->where(array('_id' => new MongoDB\BSON\ObjectID($id)))->delete('cancellationReason'.$reason);
        }
    ...
    function get_can_reasons() {
        $this->load->library('mongo_db');
        $res = $this->mongo_db->get('cancellationReasonCourier');
        return $res;
    }
    ...
    function getCanData() {
        ...
        foreach ($id as $ids) {
            $res = $this->mongo_db->where(array('_id' => new MongoDB\BSON\ObjectID($ids)))->find_one('cancellationReason'.$res);
        }
    ...
    function cancell_act() {
        ...
        if ($edit_id == '') {
            ...
            $cursor = $this->mongo_db->get('cancellationReason'.$reason);
            $data = array();
            foreach ($cursor as $res) {
                $data = $res;
            }
            ...
            $this->mongo_db->insert('cancellationReason'.$reason, array('res_id' => $edit_id, "reasons" => $reasons, 'res_for' => $res_for));
            echo json_encode(array('msg' => '1', 'insert' => $edit_id, 'reason' => $reasons));
            die;
        }else {

            $this->mongo_db->where(array('res_id' => (int) $edit_id))->set(array("reasons" => $reasons))->update('cancellationReason'.$reason);
            echo json_encode(array('msg' => '1'));
            die;
        }
...
//         }
    ...
    function getCancelResonsSupportLanguage() {
        ...
        if($type=='Courier')
        {
            $getAll = $this->mongo_db->where(array('_id' => new MongoDB\BSON\ObjectID($this->input->post('canId'))))->find_one('cancellationReasonCourier');
        }else
        {
            $getAll = $this->mongo_db->where(array('_id' => new MongoDB\BSON\ObjectID($this->input->post('canId'))))->find_one('cancellationReasonRide');
        }
