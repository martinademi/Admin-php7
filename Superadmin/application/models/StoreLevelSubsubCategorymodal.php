...
class StoreLevelSubsubCategorymodal extends CI_Model {
    ...
    function datatable_subsubcategory($param, $status) {
        ...
        $respo = $this->datatables->datatable_mongodb('storethirdCategory', array('subCategoryId' => new MongoDB\BSON\ObjectID($param), 'status' => (int) $status), 'seqID', 1); //1->ASCE -1->DESC
...
//            $arr[] = ($value['visibility'] == 1) ? 'Unhidden' : 'Hidden';
    ...
    function subSubCategory() {
        ...
        $MAsterData = $this->mongo_db->get('storethirdCategory');
        $data = array();
...
//          echo '<pre>'; print_r($data); die;
        ...
        $result=  $res = $this->mongo_db->where(array("_id" => new MongoDB\BSON\ObjectID($id)))->set($data)->update('storethirdCategory');

        $subsubcatdata = $this->mongo_db->where(array("_id" => new MongoDB\BSON\ObjectID($id)))->find_one('storethirdCategory');
        $storeId = $subsubcatdata['storeid'];
    ...
    function deleteSubSubCategory() {
        ...
        foreach ($val as $row) {
            ...
            $catdata = $this->mongo_db->where(array("_id" => new MongoDB\BSON\ObjectID($row)))->find_one('storethirdCategory');
            ...
            $result = $this->mongo_db->where(array("_id" => new MongoDB\BSON\ObjectID($row)))->set($data)->update('storethirdCategory');
    ...
    function hideSubsubCategory() {
        ...
        $val = $this->input->post('val');
        // foreach ($val as $row) {
        //     $res = $this->mongo_db->where(array("_id" => new MongoDB\BSON\ObjectID($row)))->set(array('visibility' => 0, 'visibilityMsg' => "Unhidden"))->update('storethirdCategory');
        // }
