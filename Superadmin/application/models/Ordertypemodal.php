...
class Ordertypemodal extends CI_Model {
    ...
    function data_details($status) {
        ...
        $respo = $this->datatables->datatable_mongodb('orderType', array("status" => (int) $status), 'seqId', -1);
...
//            $arr[] = $value['statusMsg'];
    ...
    function addBrand() {
        ...
        $cursor = $this->mongo_db->get("orderType");
        $arr = [];
        $arrName = [];
        ...
        if (!in_array($data['orderName'][0], $arrName)) {
            $data = $this->mongo_db->insert('orderType', $data);
    ...
    function getBrand() {
        ...
        $data = $this->mongo_db->where(array('_id' => new MongoDB\BSON\ObjectID($Id)))->find_one('orderType');
    ...
    function editBrand() {
        ...
        try {
            $data = $this->mongo_db->where(array('_id' => new MongoDB\BSON\ObjectID($Id)))->set($data)->update('orderType');
        } catch (Exception $ex) {
            print_r($ex);
        }
    ...
    function activateBrand() {
        ...
        foreach ($Id as $id) {
            $this->mongo_db->where(array('_id' => new MongoDB\BSON\ObjectID($id)))->set(array('status' => 1, 'statusMsg' => "Active"))->update('orderType');
        }
    ...
    function deactivateBrand() {
        ...
        foreach ($Id as $id) {
            $this->mongo_db->where(array('_id' => new MongoDB\BSON\ObjectID($id)))->set(array('status' => 0, 'statusMsg' => "Inactive"))->update('orderType');
        }
    ...
    function deleteSymtom() {
        ...
        foreach ($Id as $id) {
            $this->mongo_db->where(array('_id' => new MongoDB\BSON\ObjectID($id)))->set(array('status' => 2, 'statusMsg' => "Deleted"))->update('orderType');
        }
