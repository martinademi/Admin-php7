...
class Symptommodal extends CI_Model {
    ...
    function data_details($status) {
        ...
        $_POST['iColumns'] = 10;
        $_POST['mDataProp_0'] = "symptomName";
        $_POST['mDataProp_1'] = "brandDescription";
        $_POST['mDataProp_2'] = "statusMsg";

        $sl = $_POST['iDisplayStart'] + 1;

        $respo = $this->datatables->datatable_mongodb('symptom', array("status" => (int) $status), 'seqId', -1);
...
//            $arr[] = $value['statusMsg'];
    ...
    function addBrand() {
        ...
        if (count($lanCodeArr) == count($data['symptomName'])) {
            $data['name'] = array_combine($lanCodeArr, $data['symptomName']);
        } else if (count($lanCodeArr) < count($data['symptomName'])) {
            $data['name']['en'] = $data['symptomName'][0];

            foreach ($data['symptomName'] as $key => $val) {
                foreach ($lang as $lan) {
        ...
        $cursor = $this->mongo_db->get("symptom");
        $arr = [];
        $arrName = [];
        foreach ($cursor as $cdata) {
            array_push($arr, $cdata['seqId']);
            array_push($arrName, $cdata['symptomName'][0]);
        }
        $max = max($arr);
        $data['seqId'] = $max + 1;
        $data['status'] = 1;
...
//         echo '<pre>'; print_r($data); die;
        if (!in_array($data['symptomName'][0], $arrName)) {
            $data = $this->mongo_db->insert('symptom', $data);
    ...
    function getBrand() {
        ...
        $data = $this->mongo_db->where(array('_id' => new MongoDB\BSON\ObjectID($Id)))->find_one('symptom');
    ...
    function editBrand() {
        ...
        try {
            $data = $this->mongo_db->where(array('_id' => new MongoDB\BSON\ObjectID($Id)))->set($data)->update('symptom');
        } catch (Exception $ex) {
            print_r($ex);
        }
    ...
    function activateBrand() {
        ...
        foreach ($Id as $id) {
            $this->mongo_db->where(array('_id' => new MongoDB\BSON\ObjectID($id)))->set(array('status' => 1, 'statusMsg' => "Active"))->update('symptom');
        }
    ...
    function deactivateBrand() {
        ...
        foreach ($Id as $id) {
            $this->mongo_db->where(array('_id' => new MongoDB\BSON\ObjectID($id)))->set(array('status' => 0, 'statusMsg' => "Inactive"))->update('symptom');
        }
    ...
    function deleteSymtom() {
        ...
        foreach ($Id as $id) {
            $this->mongo_db->where(array('_id' => new MongoDB\BSON\ObjectID($id)))->set(array('status' => 2, 'statusMsg' => "Deleted"))->update('symptom');
        }
