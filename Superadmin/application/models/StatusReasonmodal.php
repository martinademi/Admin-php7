...
class StatusReasonmodal extends CI_Model {
    ...
    public function datatable_canreasonCustomer() {
        ...
        $respo = $this->datatables->datatable_mongodb('unsuccessful_reason', array('res_for' => 'partialSuccessfull'), '', 1);
        $aaData = $respo["aaData"];
        $datatosend = array();
    ...
    public function datatable_canreasonDriver() {
        ...
        $respo = $this->datatables->datatable_mongodb('unsuccessful_reason', array('res_for' => 'driver'), '', 1);
        $aaData = $respo["aaData"];
        $datatosend = array();
    ...
    public function datatable_canreasonDispatcher() {
        ...
        $respo = $this->datatables->datatable_mongodb('unsuccessful_reason', array('res_for' => 'manager'), '', 1);
        $aaData = $respo["aaData"];
        $datatosend = array();
    ...
    function deleteCan() {
        ...
        $this->mongo_db->where(array('res_id' => (int) $ids))->delete('unsuccessful_reason');
        echo json_encode(array('msg' => '1'));
    }

    function getDescription($cat_id = '') {
        $this->load->library('mongo_db');
        $res = $this->mongo_db->where(array('cat_id' => (int) $cat_id))->find_one('support_txt');
        if ($res['desc']) {
    ...
    function getsubDescription($cat_id = '', $subcatID = '',$languageCode='') {
        $this->load->library('mongo_db');
        $res = $this->mongo_db->where(array('cat_id' => (int) $cat_id, 'sub_cat.id' => new MongoDB\BSON\ObjectID($subcatID)))->find_one('support_txt');
    ...
    function get_can_reasons() {
        $this->load->library('mongo_db');
        $res = $this->mongo_db->get('unsuccessful_reason');
        return $res;
    }
    ...
    function getCanData() {
        ...
        foreach ($id as $ids) {
            $res = $this->mongo_db->where(array('_id' => new MongoDB\BSON\ObjectID($ids)))->find_one('unsuccessful_reason');
        }
    ...
    function cancell_act() {
        ...
        $res_for_id = $this->input->post('res_for_id');

        $whogetsPaid=$this->input->post('whoGetsPaid');
        $whoGetsCharged=$this->input->post('whoGetsCharged');
        $ChargeDeliveryFee=$this->input->post('chargeDeliveryFee');
        $returnToStore=$this->input->post('returnToStore');
...
//        echo '<pre>'; print_r($arr); die;
        if ($edit_id == '') {
            ...
            $cursor = $this->mongo_db->get('unsuccessful_reason');
            $data = array();
            foreach ($cursor as $res) {
                $data = $res;
            }
...
//            array('res_id' => $edit_id, "reasons" => $reasons, 'res_for' => $res_for);
            ...
            $arr['reasons'] = $reasons;
            $arr['res_for'] = $res_for;
            $arr['res_for_id'] =(int) $res_for_id;

            $arr['whogetsPaid'] = (int)$whogetsPaid;
            $arr['whoGetsCharged'] = (int)$whoGetsCharged;
            $arr['ChargeDeliveryFee'] =(int) $ChargeDeliveryFee;
            $arr['returnToStore'] =(int) $returnToStore;
          

          
            $this->mongo_db->insert('unsuccessful_reason', $arr);
            echo json_encode(array('msg' => '1', 'insert' => $edit_id, 'reason' => $reasons));
            die;
        }else {

            $this->mongo_db->where(array('res_id' => (int) $edit_id))->set(array("reasons" => $reasons, 'reasonObj' => $arr['reasonObj']))->update('unsuccessful_reason');
    ...
    function get_cat_supportCustomer($param = '', $param2 = '') {
        ...
        if ($param == '') {
            ...
            $res = $this->mongo_db->where(array('usertype' => "1"))->get('support_txt');
        } else {
            if ($param2 == '')
                $res = $this->mongo_db->get_where('support_txt', array('_id' => new MongoDB\BSON\ObjectID($param)));
            else
                $res = $this->mongo_db->where(array('cat_id' => (int) $param))->find_one('support_txt');
        }
    ...
    function get_cat_support($param = '', $param2 = '') {
        ...
        $this->load->library('mongo_db');
        if ($param == '') {
            $res = $this->mongo_db->where(array('usertype' => "2"))->get('support_txt');
        } else {
            if ($param2 == '')
                $res = $this->mongo_db->where(array('_id' => new MongoDB\BSON\ObjectID($param)))->get('support_txt');
            else
                $res = $this->mongo_db->where(array('cat_id' => (int) $param))->find_one('support_txt');
        }
    ...
    function get_cat_store($param = '', $param2 = '') {
        ...
        $this->load->library('mongo_db');
        if ($param == '') {
            $res = $this->mongo_db->where(array('usertype' => "3"))->get('support_txt');
        } else {
            if ($param2 == '')
                $res = $this->mongo_db->where(array('_id' => new MongoDB\BSON\ObjectID($param)))->get('support_txt');
            else
    ...
    function get_subcat_support($param = '') {
        $this->load->library('mongo_db');
        $res = $this->mongo_db->where(array('_id' => new MongoDB\BSON\ObjectID($param)))->find_one('support_txt');
    ...
    function get_cat_SubCategory($catid) {
        ...
        $this->load->library('mongo_db');
        $res = $this->mongo_db->where(array('cat_id' => (int) $catid))->find_one('support_txt');
    ...
    function support_actionCustomer() {
        ...
        if ($edit_id == '') {
            if ($cat_id == '') {
                ...
                $result = $this->mongo_db->where(array('cat_id' => (int) $cat_id))->push(array('sub_cat' => $data))->update('support_txt');
            }
        } else {
            unset($data['sub_cat']);
...
//            $data['id'] = new MongoDB/BSON/ObjectID();
            if ($scat_id == '') {
                $this->mongo_db->where(array('_id' => new MongoDB\BSON\ObjectID($edit_id)))->set($data)->update('support_txt');
            } else {
                $this->mongo_db->where(array('sub_cat.id' => new MongoDB\BSON\ObjectID($scat_id)))->pull('sub_cat', array('id' => new MongoDB\BSON\ObjectID($scat_id)))->update('support_txt');
                unset($data['sub_cat']);
                unset($data['has_scat']);
                $data['id'] = new MongoDB\BSON\ObjectID($scat_id);
                $this->mongo_db->where(array('_id' => new MongoDB\BSON\ObjectID($edit_id)))->push(array('sub_cat' => $data))->update('support_txt');
            }
        }
    }
...
//            $data['id'] = new MongoDB/BSON/ObjectID();
    ...
    function getCustomerSupportLanguage() {
        $this->load->library('mongo_db');
        $getAll = $this->mongo_db->where(array('_id' => new MongoDB\BSON\ObjectID($this->input->post('supportId'))))->find_one('support_txt');
    ...
    function getSubCustomerSupportLanguage() {
        $this->load->library('mongo_db');
        $getAll = $this->mongo_db->where(array('sub_cat.id' => new MongoDB\BSON\ObjectID($this->input->post('supportId'))))->find_one('support_txt');
        $getLanguages = $this->mongo_db->where(array('Active'=> 1))->get('lang_hlp');
        echo json_encode(array('data' => $getAll, 'lang' => $getLanguages));
    }
}
