...
//            echo $entities;
    ...
    function datatable_business($status = '') {
        ...
        foreach ($aaData as $value) {
            ...
            $arr = array();
            $arr[] = $index++;
            $arr[] = '<a target="_blank" href="' . franchiseLink . 'index.php?/Franchise/FromAdmin/' . (string) $value['_id']['$oid'] . '"  data-toggle="modal">' . implode($value['franchiseName'], ',') . ' </a>';
...
//        echo json_encode(array("_id" => $val ));
    ...
    function validateEmail() {
        $this->load->library('mongo_db');
        $res = $this->mongo_db->where(array('email' => $this->input->post('email')))->get('franchise');
        $cout = count($res);
