...
class Deliveryschduledmodal extends CI_Model {
    ...
    public function datatable_getProviderDetails($status = '',$timeOffset = '') {
        ...
        switch((int)$status){
            case 1:
            $respo = $this->datatables->datatable_mongodb('zonesDeliveryWorking',array('storeId'=>$this->session->userdata('badmin')['BizId'],'status'=> 1));
            break;
            case 2:
            $respo = $this->datatables->datatable_mongodb('zonesDeliveryWorking', array('storeId'=>$this->session->userdata('badmin')['BizId'],'status' => 2));
            break;
            case 3:
            $respo = $this->datatables->datatable_mongodb('zonesDeliveryWorking', array('storeId'=>$this->session->userdata('badmin')['BizId'],'status' => 3));
            break;
