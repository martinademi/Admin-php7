...
class Deliveryassigndrivermodal extends CI_Model {
    ...
    public function datatable_getProviderDetails($zoneId,$status,$timeOffset,$zoneDeliveryId) {
        ...
        $respo = $this->datatables->datatable_mongodb('driver',array("serviceZones" => $zoneId));
        ...
        $datatosend = array();
            ...
            foreach($aaData as $value){
                ...
                $driverId=$value['_id']['$oid'];
                // $OCapacity = $this->mongo_db->where(array('_id'=> new MongoDB\BSON\ObjectID($zoneDeliveryId)))->select(array('driver'=>'driver'))->get('zonesDeliverySlots');
                $OCapacity= $this->mongo_db->aggregate('zonesDeliverySlots',[
                   [ '$match'=>['_id'=> new MongoDB\BSON\ObjectID($zoneDeliveryId) ]],
                   ['$unwind'=>'$driver'],
                ...
                $doc=[];
                foreach( $OCapacity as $oCap){
                    $doc=json_decode(json_encode($oCap),true);
                }
                ...
                $arr[] ="<div class='col-sm-4' style='padding:0'><input type='text' class='quantity' id='" . $value['_id']['$oid'] . "' value='". $doc['driver']['orderCapacity']."' onkeypress='return isNumberKey(event,this)' style='display:inline; width:100%;' placeholder='Enter Order Quantity' disabled ></div>";
                $arr[] = "<a class='btn btn-danger  cls111 deleteDriverSlot' style='width:35px;' data-id='" . $value['_id']['$oid'] . "'><i class='fa fa-trash'></i></a>";
                $arr[] = "<input type='checkbox' class='checkbox checkboxProduct' id='checkboxProduct'  data='" . $value['_id']['$oid'] . "' value='" . $value['_id']['$oid'] . "'>";
    ...
    function getDriver(){
        ...
        //$data = $this->mongo_db->where(array('serviceZones'=>$zoneId,'status'=> 9))->get('driver');
        $data = $this->mongo_db->where(array('status'=> 9))->select(array('firstName'=>'firstName','lastName'=>'lastName','mobile'=>'mobile','countryCode'=>'countryCode'))->get('driver');
    ...
    function addDriver(){		
        $this->load->library('mongo_db');
        $id = $this->input->post('driverval');
        $name = $this->input->post('driverName');
        $orderCapacity =(int) $this->input->post('orderCapacity');
        $countryCode = $this->input->post('countryCode');
        $mobile = $this->input->post('mobile');
        $zoneDeliveryId =$this->input->post('zoneDeliveryId');
        $listInZone =$this->input->post('listInZone');
        $assignOrder=[];
        if($listInZone==1){
            $listInZone=true;
        }else{
        ...
        $result = array('id' => $id,'drivername'=>$name,'orderCapacity'=>$orderCapacity,'number'=> $number,'listInZone'=>$listInZone,'assignedOrder'=>$assignOrder );
        $data = $this->mongo_db->where(array('_id' =>new MongoDB\BSON\ObjectID($zoneDeliveryId)))->push(array('driver'=>$result))->update('zonesDeliverySlots'); 
        echo json_encode(array('data'=>$data,"flag"=>1));

     }


        function AddNewDriverSlot($zoneDeliveryId) {

            $this->load->library('mongo_db');
            $oQuantity=$this->input->post('oQuantity');
            $assignOrder=[];
           
            foreach($oQuantity as $key=>$value){
                
                 $driverDatadetail=$this->mongo_db->where(array('_id'=>new MongoDB\BSON\ObjectID($zoneDeliveryId),'driver.id'=>$key))->find_one('zonesDeliverySlots');
                 if($driverDatadetail){
                    //  update
                    $data=$this->mongo_db->where(array('_id'=>new MongoDB\BSON\ObjectID($zoneDeliveryId),'driver.id'=>$key))->set(array('driver.$.orderCapacity'=>(int)$value))->update('zonesDeliverySlots');
                 }else{

                    $driverData=$this->mongo_db->where(array('_id'=>new MongoDB\BSON\ObjectID($key)))->select(array('firstName'=>'firstName','lastName'=>'lastName','mobile'=>'mobile','countryCode'=>'countryCode'))->find_one('driver');
                    $name=$driverData['firstName'].' '.$driverData['lastName'];
                    $number=$driverData['countryCode'].' '.$driverData['mobile'];
                    $result = array('id' => $key,'drivername'=>$name,'orderCapacity'=>(int)$value,'number'=> $number,'assignedOrder'=>$assignOrder );
                    $data = $this->mongo_db->where(array('_id' =>new MongoDB\BSON\ObjectID($zoneDeliveryId)))->push(array('driver'=>$result))->update('zonesDeliverySlots'); 
                 }
        ...
        function deleteDriverSlot($zoneDeliveryId,$driverId){
            $this->load->library('mongo_db');
            
           $data = $this->mongo_db->where(array('_id' =>new MongoDB\BSON\ObjectID($zoneDeliveryId)))->pull('driver',array('id'=>$driverId))->update('zonesDeliverySlots');
