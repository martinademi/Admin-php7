...
class Deliveryslotsmodal extends CI_Model {
    ...
    public function datatable_getProviderDetails($zoneId,$status = '',$timeOffset = '') {
        ...
        date_default_timezone_set("Asia/Kolkata");
           ...
           $dateTimestamp = strtotime($date);
            $workingHourStoreDoc = $this->mongo_db->where(array('status'=> 1,'storeId'=>$this->session->userdata('badmin')['BizId'], 'endDateTimestamp'=>['$lt'=>$dateTimestamp]))->get('zonesDeliveryWorking') ;
        ...
        switch((int)$status){
            case 1:
            $respo = $this->datatables->datatable_mongodb('zonesDeliverySlots',array('zoneId'=>$zoneId,'status'=> 1));
            break;
            case 2:
            $respo = $this->datatables->datatable_mongodb('zonesDeliverySlots',array('zoneId'=>$zoneId,'status'=> 1));
            break;
        ...
        $driverCount=0;
        $countres = $this->mongo_db->aggregate('driver', array(array('$match'=>array('status' => 9,'serviceZones'=>$zoneId)),array('$group'=>array('_id'=>1,count=>array('$sum'=>1)))));
        foreach($countres as $res){
            $driverCount= json_decode(json_encode($res->count,true));
        ...
        foreach ($aaData as $value) {
            ...
            $totalOrdeCapacity=0;
            $assignedOrder=0;
            $driverCount=count($value['driver']);
            foreach($value['driver'] as $driver){
                $totalOrdeCapacity+=$driver['orderCapacity'];

            }

            $startDateTimeForClient = date("Y-m-d H:i:s",strtotime($timeOffset.'minutes',strtotime($value['startDateTimestamp'])));
            $endDateTimeForClient = date("Y-m-d H:i:s",strtotime($timeOffset.'minutes',strtotime($value['endDateTimestamp'])));

            $orderCount=5;

            $statusmsg=($value['status']==1) ? 'Active': 'Exhausted';
            $arr[] = $sl++;  
            $arr[] = $value['date'];          
            $arr[] = $value['startTime'];         
            $arr[] = $value['endTime'];
            $arr[] = ($orderCount!=0) ? '<a class="orderCapacity" data-id="'.$value['city_ID'].'" data-zoneid="'.$zoneId.'" value="' . $value['_id']['$oid'] . '" style="color: royalblue; cursor: pointer;"  >' .  $assignedOrder.'/'.$totalOrdeCapacity . '</a>' : '0';
            $arr[] = ($driverCount!=0) ? '<a class="driverdeliverySlots" data-id="'.$value['_id']['$oid'].'" data-zoneid="'.$zoneId.'" value="' . $value['_id']['$oid'] . '" style="color: royalblue; cursor: pointer;"  >' . $driverCount . '</a>' : '0';
            $arr[] = $statusmsg;
            $arr[] = "<button class='btn driverAssignSlot btn-primary cls111' style='width:35px; border-radius: 25px;' data-id='" . $value['city_ID'] . "' data-zoneid='".$zoneId."' value='" . $value['_id']['$oid'] . "' ><i class='fa fa-user-plus' style='font-size:12px;'></i> </button>".
                      "<button class='btn editSlot btn-primary cls111' style='width:35px; border-radius: 25px;' data-id='" . $value['_id']['$oid'] . "'><i class='fa fa-edit' style='font-size:12px;'></i> </button>".
                      "<button class='btn dltSlot btn-primary cls111' style='width:35px; border-radius: 25px;' data-id='" . $value['_id']['$oid'] . "'><i class='fa fa-trash' style='font-size:12px;'></i> </button>";
