...
class brandsmodal extends CI_Model {
    ...
    function data_details($status) {
        ...
        $respo = $this->datatables->datatable_mongodb('centralBrands', array("status" => (int) $status), '_id', -1);
...
//         echo '<pre>'; print_r($data); die;
        if (!in_array($data['brandName'][0], $arrName)) {
            $data = $this->mongo_db->insert('centralBrands', $data);
    ...
    function getBrand() {
        ...
        $data = $this->mongo_db->where(array('_id' => new MongoDB\BSON\ObjectID($Id)))->find_one('centralBrands');
    ...
    function editBrand() {
        ...
        try {
            $data = $this->mongo_db->where(array('_id' => new MongoDB\BSON\ObjectID($Id)))->set($data)->update('centralBrands');
    ...
    function activateBrand() {
        ...
        foreach ($Id as $id) {
            $data=   $this->mongo_db->where(array('_id' => new MongoDB\BSON\ObjectID($id)))->set(array('status' => 1, 'statusMsg' => "Active"))->update('centralBrands');
    ...
    function deactivateBrand() {
        ...
        foreach ($Id as $id) {
           $data= $this->mongo_db->where(array('_id' => new MongoDB\BSON\ObjectID($id)))->set(array('status' => 0, 'statusMsg' => "Inactive"))->update('centralBrands');
