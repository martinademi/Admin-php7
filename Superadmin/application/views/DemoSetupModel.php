...
class DemoSetupModel extends CI_Model {
    ...
    public function datatablePaymentGateway($status) {
        ...
        switch($status){
            case 1;
                $respo = $this->datatables->datatable_mongodb('store',array(),'',-1);
                break;
            case 2;
                $respo = $this->datatables->datatable_mongodb('store',array(),'',-1);
                break;
...
//        $respo = $this->datatables->datatable_mongodb('paymentGateway',array('isDeleted' => false),'',-1);
        ...
        foreach ($aaData as $value) {
            ...
            $arr[] = '<button class="btn btn-primary btnWidth editPaymentGateway"  value=' . $value['_id']['$oid'] . '><i class="fa fa-edit"></i></button>';
