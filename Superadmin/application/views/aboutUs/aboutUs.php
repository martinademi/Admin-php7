...
<script>
    function uploadImage(formElement, id, imageId, hiddenId) {
        var form_data = new FormData();
        form_data.append("OtherPhoto", formElement)
        $.ajax({
            url: "<?php echo base_url('index.php?/Trending') ?>/imageUpload",
            type: "POST",
            data: form_data,
            dataType: "JSON",
            beforeSend: function () {
...
//                    '        <input type="file" attribut="' + visi + '" attribute_id="' + visi + '" name="imageupload"' + visi + '" id="imageupload" class="error - box - class form - control imagesProduct"  data-attribute="job0" id="jobImages" attrId="0" value="Text Element 1">' +
                            '<input type="file" attribute_id="' + inv + '" name="imageupload"  id="imageupload" class="error-box-class form-control imageupload"  data-attribute="job0" id="jobImages" attrId="0" value="Text Element 1">' +
                            '   </div>' +
                            '  <div class="col-sm-2">' +
                ...
                for (var i = 0; i < result.investor.length; i++) {
                    var divInvestor = '<br><br>' +
                            ...
                            '     <div class="col-sm-6">' +
                            '        <input type="file" attribute_id="' + inv + '" name="imageuploadInv" id="imageuploadInv" class="error-box-class form-control imageuploadInvClass"  data-attribute="job0" id="jobImages" attrId="0" value="Text Element 1">' +
                            '   </div>' +
                            '  <div class="col-sm-2">' +
    ...
    $(document).ready(function () {
        ...
        $(document).on('change', '.imageuploadclass', function () {
            ...
            uploadImage(formElement, id, "#showjob", "#jobimages");
        });
        $(document).on('change', '.imageuploadInvClass1', function () {
            ...
            uploadImage(formElement, id, "#showjobInv", "#jobimagesInv");
//            console.log("attId", this);
//            var id = $(this).attr('attribute_id');
//            console.log('id',id);
