...
//   });
         ...
         $(":file").on("change", function(e) {
            ...
            else
            {
                ...
                var folderName;
                 switch($(this).attr('id'))
                {
                    case "file_upload": type = 1;
                                          folderName = 'ProfilePics';  
                                          break;
                    case "id_proof": type = 3;
        ...
        //                    $("#ImageLoading").show();
                        ...
                        success: function (result) {
                              ...
                              switch(type)
                            {
                                case 1: $('#driverImage').val('<?php echo AMAZON_URL;?>'+result.fileName);
                                        $('.driverImage').attr('src','<?php echo AMAZON_URL;?>'+result.fileName);
                                        $('.driverImage').show();
                                        break;
                                case 2: $('#driverLicence').val('<?php echo AMAZON_URL;?>'+result.fileName);
                                        $('.driverLicence').attr('src','<?php echo AMAZON_URL;?>'+result.fileName);
                                        $('.driverLicence').show();
                                         break;
                                case 3: $('#idproof').val('<?php echo AMAZON_URL;?>'+result.fileName);
      ...
      function phoneValidationCheck(phone,coutryCode)
      {
          ...
          OnlymobileNo = phone;
          CountryCodeMobileNo = '+'+coutryCode+'-'+phone;
         var returnStatus;
            var status = '1';
            $.ajax({
...
//validations for each previous tab before proceeding to the next tab
    ...
    function addresstab(litabtoremove, divtabtoremove)
    {
        ...
        {
            var paswordEncrypted = $(".paswordEncrypted").val();
            if(!paswordEncrypted)
            {
                    ...
                    $.ajax({
                       ...
                       data:{password:$('#password').val()},
                        success: function (result) {
                            ...
                            {
                                $('.paswordEncrypted').val(result.password);
...
//                alert("complete Login Details tab properly")
    ...
    function bonafidetab(litabtoremove, divtabtoremove)
    {
        ...
        if (addresstab(litabtoremove, divtabtoremove))
        {
            ...
            if ($("#driverLicence").val() == '' || $("#expirationrc").val() =='')
            {
                bstatus = false;
					...
					<ul
						...
						<li class="tabs_active" id="fourthlitab"><a data-toggle="tab"
							href="#tab4" onclick="bonafidetab('fourthlitab', 'tab4')"><i
								id="tab4icon" class=""></i> <span>PREFERRED ZONES</span></a></li>
						...
						<div class="tab-content">
							<div class="tab-pane padding-20 slide-left active" id="tab1">
								<div class="row row-same-height">
									...
									<div class="form-group">
										...
										<div class="col-sm-6">
											<select id="company_select" name="company_select"
												...
												type="hidden" id="operatorName" name="operatorName"
									...
									<div class="form-group">
										<label for="" class="control-label col-sm-2">Plan<span
											style="" class="MandatoryMarker"> *</span></label>
										<div class="col-sm-6">
											<select class="form-control" id="plan" name="plan">
												<option value="">Select</option>
                                                        ...
                                                        foreach ($plans as $each) {
                                                            echo "<option value='" . $each['_id']['$oid'] . "' plan='" . $each['plan_name'] . "'>" . $each['plan_name'] . "</option>";
                                                        }
                                                        ?>
                                                    </select>
										</div>
							...
							<div class="tab-pane slide-left padding-20" id="tab2">
								<div class="row row-same-height">
									...
									<div class="form-group">
										...
										<div class="col-sm-6">
											<input type="tel" data="1" id="mobile" name="mobile"
												required="required" class="form-control"
							...
							<div class="tab-pane slide-left padding-20" id="tab3">
								<div class="row row-same-height">
									<div class="form-group">
										<label for="fname" class="col-sm-2 control-label"><?php echo FIELD_DRIVERS_UPLOADDRIVERLICENSE; ?><span
											style="" class="MandatoryMarker"> *</span></label>

										<div class="col-sm-5">
											<input type="file" class="form-control" style="height: 37px;"
												name="certificate" id="file_upload_l"> <input type="hidden"
												class="form-control" style="height: 37px;"
							...
							<div class="tab-pane slide-left padding-20" id="tab4">
								<div class="row row-same-height">
                                    ...
                                    <?php
                                    foreach ($cityForZones as $zone) {
                                        ?>
                                    
                                    <div class="form-group">
										<label for="fname" class="col-sm-2 control-label"><?php echo $zone['city']?></label>
										<div class="col-sm-6">
											<input id="" name="checkboxs[]"
												value="<?php echo $zone['_id']['$oid']?>"
												required="required" type="checkbox" class="checkbox">
										</div>
										<div class="col-sm-3 error-box" id="file_driver_exdate"></div>
									</div>
                                    <?php
