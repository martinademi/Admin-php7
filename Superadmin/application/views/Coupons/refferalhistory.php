...
<script>
    ...
    var table = $('#big_table');       
        var settings = {
         ...
         "bProcessing": true,
         "bServerSide": true,
         "sAjaxSource": '<?php echo base_url() ?>index.php/coupons/datatable_refferalHistory/<?= $refId?>',
         "bJQueryUI": true,
  ...
  $('#searchData').click(function(){
            if ($("#start").val() && $("#end").val())
            {
                ...
                settings.sAjaxSource = '<?php echo base_url() ?>index.php/coupons/datatable_refferalHistory/<?= $refId?>/'+ st + '/' + end;
                his_tbl.dataTable(settings);
            }else{
                swal({
                    title: "Error",
