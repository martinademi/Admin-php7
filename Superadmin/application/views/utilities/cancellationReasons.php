<script type="text/javascript">
    $(document).ready(function () {
        ...
        var settings = {
            ...
            "bProcessing": true,
            "bServerSide": true,
            "sAjaxSource": '<?php echo base_url() ?>index.php?/utilities/datatable_canreason/<?= $reason?>/<?= $reasonFor?>',
            "bJQueryUI": true,
...
//                  $('#big_table_processing').hide();
        ...
        $('#searchlanguage').change(function () {
            ...
            "bProcessing": true,
            "bServerSide": true,
            "sAjaxSource": (optionSelected == 'All')?('<?php echo base_url() ?>index.php?/utilities/datatable_canreason/<?= $reason?>/<?= $reasonFor?>'):('<?php echo base_url() ?>index.php?/Utilities/transection_data_ajax/<?= $reason?>/<?= $reasonFor?>/' + optionSelected),
            "bJQueryUI": true,
            "sPaginationType": "full_numbers",
            "iDisplayStart ": 20,
            "oLanguage": {
...
//            $("#callone").trigger("click");
        ...
        $(document).on('click', '.cancelReson', function ()
        {
            ...
            $.ajax({
                type: "POST",
                url: "<?php echo base_url('index.php?/utilities') ?>/getCancelResonsSupportLanguage",
                ...
                success: function (result) {
                    ...
                    html += "<td style='text-align: center;'>" + result.data.reasons.en + "</td></tr>";

                    $.each(result.lang, function (i, res) {
                        if (result.data.reasons[res.code]) {
                            html += "<tr><td style='text-align: center;text-transform: capitalize;'>"+res.name+"</td>";
                            html += "<td style='text-align: center;'>" + result.data.reasons[res.code] + "</td></tr>";
                        }
                    })
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <?php foreach($language as $value):?>
                        ...
                        <input type="text" lang = "<?= $value['code'];?>" id="addReasons" class="form-control addReasons" placeholder="Enter the reason in <?= ucwords($value['name']);?> for cancellation"/>
