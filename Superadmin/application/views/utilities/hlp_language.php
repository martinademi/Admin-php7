    ...
    //   base_url().'index.php?/superadmin/access_denied';
        ...
        setTimeout(function () {
            var settings = {
                ...
                "bProcessing": true,
                "bServerSide": true,
                "sAjaxSource": '<?php echo base_url() ?>index.php?/utilities/datatable_language',
                "bJQueryUI": true,
        ...
        $('#add_lan').click(function () {
            $('#errorboxADD').text('');
            if ($('#languageList').val() == '') {
                $('#languageList').closest('.form-group').addClass('has-error');
                $('#errorboxADD').text("Please select a language");
                return;
            }
            if ($('#languageList').val() == 'english' || $('#languageList').val() == 'English' || $('#languageList').val() == 'ENGLISH') {
                $('#errorboxADD').text("English is default language.. Please enter another Language");
                return;
            }

            $('#languageList').closest('.form-group').removeClass('has-error');
            var edit_id = $('#edit_id').val();

            var lang = $('#languageList option:selected').val();
            var lang_code = $('#languageList option:selected').attr("data-code");

            $.ajax({
                url: "<?php echo base_url() ?>index.php?/utilities/lan_action",
                type: "POST",
                data: {lan_name: lang, langCode: lang_code, lan_id: edit_id},
//                data: $('#lan_form').serialize(),
                ...
                success: function (result) {
                    ...
                    if (result.msg == '1') {
                        ...
                        $('#errorLog').text('Language already exists..!');
                    } else {
        ...
        $(document).on('click', '.btnedit', function () {
            ...
            console.log(rowid);
            $.ajax({
                url: "<?php echo base_url() ?>index.php?/utilities/get_lan_hlpTextone/" + rowid,
                type: "POST",
                dataType: "JSON",
                success: function (result) {
//                    console.log(result._id.$oid);
                    $('#languageList').val(result.lan_name);
                    $('#edit_id').val(result.lan_id);
                }
            });
            $('#lan_title').html('Update Language');
            $('#add_lan').html('Update');
            $('#modal-lan').modal('show');
        });
...
//                        }, 500);
        ...
        $("#confirmeds").click(function () {
            var id = $(this).val();
            $.ajax({
                url: "<?php echo base_url('index.php?/utilities') ?>/enable_lang",
                type: 'POST',
                data: {id: id},
                dataType: 'json',
                success: function (result)
...
//                            console.log(result);
        ...
        $("#confirmeds1").click(function () {
            var id = $(this).val();
            $.ajax({
                url: "<?php echo base_url('index.php?/utilities') ?>/disable_lang",
                type: 'POST',
                data: {id: id},
                dataType: 'JSON',
                success: function (result)
                ...
                <div class="panel panel-transparent ">
                    ...
                    <div class="container-fluid container-fixed-lg bg-white col-md-12 col-sm-12 col-xs-12" style="margin-top: 1%;">
                        <div class='pull-right'>
                            <button class='btn btn-primary cls110' id='add_new_lan' style="margin: 10px 5px; border-radius: 25px;">
                                Add 
                            </button>
                        </div>
                    </div>
                        ...
                        <div class="row">
                            <div class="col-sm-12">
                                <div class="form-group form-group-default required">
                                    <label>Language<span class="MandatoryMarker">*</span></label>

                                    <select id="languageList" name='language' class='form-control error-box-class' <?PHP echo $enable; ?> >
                                        <option value="">Select language</option>
                                        <?php
                                        foreach ($allLanguage['languages'] as $result) {
                                            echo "<option value='" . $result['name'] . "' data-code='" . $result['code'] . "'>" . $result['name'] . "</option>";
                                        }
                                        ?>
                                    </select>
                                ...
                                <div class="error-box" id="errorboxADD" style="font-size: 14px;text-align:center;color:red;"></div>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <div class="row">
                    ...
                    <div class="col-sm-4"></div>
                    <div class="col-sm-4" >
                        <button type="button" class="btn btn-primary pull-right" id="confirmeds1" value="" style="margin:0;"><?php echo BUTTON_YES; ?></button>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="row">
                    ...
                    <div class="error-box" id="errorbox_logout" style="font-size: large;text-align:center">Successfully Deleted..!!!</div>
...
<div class="modal fade stick-up" id="popUpAlert" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
            ...
            <div class="modal-body">
                ...
                <div class="row">
                    ...
                    <div class="error-box modalPopUpText" id="errorLog" style="text-align:center;color:red;">Language already exists..!</div>
