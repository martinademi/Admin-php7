    ...
    $(document).ready(function () {
        ...
        $.each(lan, function (ind, val) {
            ...
            var html = '<div class="form-group form-group-default required cat_lan_' + val.lan_id + '">\
                            ...
                            <input type="text" required name="cat_name[' + val.lan_id + ']" id="cat_name_' + val.lan_id + '" value="' + data.name[val.lan_id] + '" class="form-control" >\
                        </div>';
        ...
        if (data.has_scat == false) {
            ...
            $.each(lan, function (ind, val) {
                ...
                var html = '<div class="form-group form-group-default required cat_lan_' + val.lan_id + '">\
                            ...
                            <textarea required name="desc[' + val.lan_id + ']" id="desc_name_' + val.lan_id + '" class="form-control nhas_scat" rows="5">' + data.desc[val.lan_id] + '</textarea>\
                        </div>';
                $(html).insertAfter($('.maindesc'));
                if (data.cat_hform == true) {
                    $.each(data.form_fields, function (ind, val) {
                        ...
                        <div class="row main_field" style="margin-top:10px;" data-id="' + inputtextId + '">\
                            <div class="form-group form-group-default required col-sm-5" style="padding:7px 4px 12px 12px !important;">\
                                ...
                                <input type="text" class="form-control" name="lbl[' + inputtextId + '][0]" value="' + val[0][0] + '" required>\
                            </div>\
                            <div class="form-group form-group-default required col-sm-4" style="padding:7px 4px 12px 12px !important;">\
                                ...
                                <select class="form-control" id="dtype[' + inputtextId + ']" name="dtype[' + inputtextId + ']">\
                                    <option value="0" ' + ((val[1] == 0) ? 'selected' : '') + '>Text</option>\
                                    <option value="1" ' + ((val[1] == 1) ? 'selected' : '') + '>Date</option>\
                                </select>\
                            </div>\
                            <div class="col-sm-3 checkbox" style="padding:7px 4px 12px 12px !important;">\
                                <input ' + ((val[2] == 1) ? 'checked' : '') + ' type="checkbox" value="1" name="Mand[' + inputtextId + ']" id="Mand[' + inputtextId + ']" data="Mand[' + inputtextId + ']" class="mandatory_or_not">\
                                ...
                                $('#cat_form_fields').append('\
                                         <div class="row cat_lan_' + $(this).val() + '">\
                                             <div class="form-group form-group-default required col-sm-5" style="padding:7px 4px 12px 12px !important;">\
                                                 ...
                                                 <input type="text" class="form-control" name="lbl[' + inputtextId + '][' + $(this).val() + ']" value="' + val[0][$(this).val()] + '" required>\
                                             </div>\
                                         </div>');
                            }
                        });
        ...
        $('.lan_check').change(function () {
            if ($(this).is(':checked')) {
                ...
                html = '<div class="form-group form-group-default required cat_lan_' + $(this).val() + '">\
                            <label>Description (' + $(this).attr('data-id') + ')</label>\
                            <textarea required name="desc[' + $(this).val() + ']" id="desc_name" class="form-control nhas_scat" rows="5"></textarea>\
                        </div>';
                $(html).insertAfter($('.maindesc'));
...
<div class="content">
    ...
    <div class="container-fluid container-fixed-lg">
        <ul class="breadcrumb">
            <li>
                <a href="<?= base_url() ?>index.php?/utilities/helpText">Help Text</a>
            </li>
            <li>
                <a href="#" class="active">
        ...
        <div class="container-md-height m-b-20">
            <div class="panel panel-default">
                <form action="<?= base_url() ?>index.php?/utilities/help_cat_action" method='post'>
...
<?php } ?>
                                    <div class='has_scat'>
                                        <div class="form-group form-group-default required maindesc">
                                            ...
                                            <textarea required name='desc[0]' id="desc_name" class="form-control nhas_scat" rows="5"></textarea>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class='col-sm-6'>
                                    <div class='has_scat'>
                                        ...
                                        <div class="form-group form-group-default input-group">
                                            ...
                                            <span class="input-group-addon bg-transparent">
                                                <input <?= $s ?> type="checkbox" data-init-plugin="switchery" data-size="small" data-color="primary" name="cat_hform" id="cat_hform"/>
                                            </span>
                                        </div>
                                                ...
                                                <div class="form-group form-group-default required">
                                                    ...
                                                    <select required name='zGroup' id="cat_form_group" class="form-control">
                                                        ...
                                                        foreach ($group as $val) {
                                                            $z = '';
                                                            if ($s != '' && $val['grp_id'] == $data['zGroup'])
                                                                $z = 'selected';
                                                            ?>
                                                            <option <?= $z ?> value='<?= $val['grp_id'] ?>'><?= $val['grp_name'] ?></option>
<?php } ?>
                                                    </select>
                                                </div>
                                                <div class="col-sm-12">
