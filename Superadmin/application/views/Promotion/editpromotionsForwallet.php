...
<script>
    ...
    var campaningID = '<?= (string)$canpaningData[0]['_id'] ?>';
    var PromoData = campaningData.PromoData;
...
//        CKEDITOR.replace('editor1');
        ...
        $('#selectedcityNew option[value=' + PromoData.city + ']').prop("selected", true);
        $('#Promocode').val(PromoData.promocode);
        $('#amount').val(PromoData.Amount);
        
        var stdt = campaningData.stdate;
        stdt = stdt.split("|");
        $('#startDate').val(stdt[0]);
        $('#starttime').val(stdt[1]);
        $('#starttime').wickedpicker({  twentyFour: true,now: stdt[1] });
        
        var enddt = campaningData.stdate;
        enddt = enddt.split("|");
        $('#EndDate').val(enddt[0]);
        $('#endtime').val(enddt[1]);
        $('#endtime').wickedpicker({  twentyFour: true,now: enddt[1] });
        
        $('#noofclaims').val(PromoData.noofclaims);
        
        $("#summernote").html(campaningData.TnC);
        
        if(PromoData.ZoneSelected == '1'){
//            $("#zonecheck").prop('checked',true);
            calltrigger('zonecheck');
            $('#updateZone option[value=' + PromoData.ZoneId + ']').prop("selected", true);
            $("input[name=zonecheck][value=" + PromoData.zoneappliedOn + "]").prop('checked',true);
        }

        $("#creditwalletWithaddition").val(PromoData.creditwalletWithaddition);
        ...
        $('.submitDataOnserver').click(function () {
            ...
            if (promocode == '') {
                ...
                $.ajax({
                    url: "<?php echo base_url() ?>index.php?/Promotions/UpdateRefferal",
                    type: "POST",
                    data: {
                    ...
                    success: function (result) {
                        if (result.msg == '0') {
                            $('.content').waitMe('hide');
                            window.location = "<?php echo base_url() ?>index.php?/Promotions/promotion/2";
            ...
            <div class="container-fluid container-fixed-lg sm-p-l-20 sm-p-r-20">
                <div class="inner">
                    ...
                    <ul class="breadcrumb" style="">
                        <li>
                            <a href="<?php echo base_url() . 'index.php?/Promotions/promotion/2'; ?>">WALLET PROMO</a>
                        </li>
                                ...
                                <div class="col-md-12 col-md-12 b-b  b-l b-r b-grey p-b-10 p-t-10 ">
                                    <div class="col-sm-6">
                                        <div class="form-group">
                                            ...
                                            <div class="col-sm-7">
                                                <input type="text" class="addmemclass1 form-control" id="creditwalletWithaddition"    placeholder="additional  amount"   required="" aria-required="true">                                                                                            
                                            </div>
